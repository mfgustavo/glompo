...
(:obj:`&`) and or (:obj:`|`) to tailor various conditions. For example::
...
.. note::
   ...
   For performance and to allow conditionality, conditions are evaluated 'lazily' i.e. :obj:`x or y` will return if `x`
   is :obj:`True` without evaluating `y`. :obj:`x and y` will return :obj:`False` if `x` is :obj:`False` without
   evaluating `y`.
