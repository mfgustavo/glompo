...
class OptimizerLogger:
    ...
    def __init__(self, path: Union[str, Path], checksum: str, n_parms: int, expected_rows: int, refresh: bool):
        ...
        self._hunting_cache = Cache()  # In memory copy of iteration results to stops reads from disk and speed hunting
    ...
    def len(self, opt_id: int) -> int:
        if opt_id in self._hunting_cache:
            return len(self._hunting_cache.get_history(opt_id, 'fx'))
    ...
    def add_optimizer(self, opt_id: int, opt_type: str, t_start: datetime.datetime):
        ...
        self.pytab_file.create_vlarray(where=f'/optimizer_{opt_id}',
                                       ...
                                       expectedrows=3)
        self._hunting_cache.add_optimizer(opt_id)
        ...
        for key, val in zip(('opt_id', 'opt_type', 't_start'),
            ...
            self._hunting_cache.put_metadata(opt_id, key, val)
    ...
    def put_iteration(self, iter_res: IterationResult):
        ...
        if iter_res.fx < self._best_iters[opt_id]['fx']:
            ...
            self._best_iters[opt_id]['call_id'] = self._f_counter

            if iter_res.fx < self._best_iter['fx']:
                self._best_iter = self._best_iters[opt_id]
    ...
    def put_metadata(self, opt_id: int, key: str, value: Any):
        ...
        if opt_id in self._hunting_cache:
            self._hunting_cache.put_metadata(opt_id, key, value)
    ...
    def get_metadata(self, opt_id, key: str) -> Any:
        ...
        if opt_id in self._hunting_cache:
            return self._hunting_cache.get_metadata(opt_id, key)
    ...
    def flush(self, opt_id: Optional[int] = None):
        ...
        for opt in opt_ids:
            if opt not in self._writing_chunk or not self.has_iter_history(opt_id):
                continue
    ...
    def clear_cache(self, opt_id: Optional[int] = None):
        ...
        for o in opt_ids:
            if o in self._writing_chunk:
                self.flush(opt_id)
                del self._writing_chunk[opt_id]

            if o in self._hunting_cache:
                self._hunting_cache.clear_cache(opt_id)
    ...
    def close(self):
        ...
        self._hunting_cache.clear_cache()
