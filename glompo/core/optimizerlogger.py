...
from ..common.helpers import glompo_colors, rolling_best
...
class OptimizerLogger:
    ...
    def __init__(self, path: Union[str, Path], checksum: str, n_parms: int, expected_rows: int, refresh: bool):
        ...
        if refresh:
            # Fresh log file
            self.pytab_file.root._v_attrs.f_counter = 0
            self.pytab_file.root._v_attrs.best_call_id = 0
            self.pytab_file.root._v_attrs.opts_started = 0

            self._best_iters = {0: {'opt_id': 0, 'x': [], 'fx': float('inf')}}
            self._best_iter = {'opt_id': 0, 'x': [], 'fx': float('inf')}
        else:
            # Loaded log file
            self._best_iters = self.pytab_file.root.best_iters
            self._best_iter = self.pytab_file.root.best_iter
    ...
    def n_optimizers(self):
        return self.pytab_file.root._v_attrs.opts_started

    def len(self, opt_id: int) -> int:
        if 'iter_hist' in self[opt_id]:
            return self[opt_id]['iter_hist'].nrows
        return 0
    ...
    def add_optimizer(self, opt_id: int, opt_type: str, t_start: datetime.datetime):
        ...
        self.pytab_file.create_vlarray(where=f'/optimizer_{opt_id}',
                                       ...
                                       expectedrows=3)
        group._v_attrs.opt_id = opt_id
        group._v_attrs.opt_type = opt_type
        group._v_attrs.t_start = t_start
        group._v_attrs.best_call_id = 0

        self.pytab_file.root._v_attrs.opts_started += 1
    ...
    def put_iteration(self, iter_res: IterationResult):
        ...
        if iter_res.fx < self._best_iters[opt_id]['fx']:
            ...
            self[opt_id]._v_attrs.best_call_id = self._f_counter
        ...
        table.append([(self._f_counter, iter_res.iter_id, iter_res.x, iter_res.fx, *iter_res.extras)])
        table.flush()
    ...
    def get_history(self, opt_id: int, track: str) -> List:
        ...
        """
        if 'iter_hist' in self[opt_id]:
            return self[opt_id]['iter_hist'].col(track)
        return []
