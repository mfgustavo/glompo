...
def make_winding_stair_trajectory(k: int, levels: int, groupings: Optional[np.ndarray] = None,
    ...
    -------
    numpy.ndarray
        Sequence of :math:`k + 1` vectors in `k` dimensional unit space representing a Manhattan style trajectory
        through it.
...
def make_radial_trajectory(k: int, groupings: Optional[np.ndarray] = None,
    ...
    -------
    numpy.ndarray
        Sequence of :math:`k + 1` vectors in `k` dimensional unit space representing a collection of points sampled
...
def make_radial_trajectory_set(r: int, k: int, groupings: Optional[np.ndarray] = None) -> np.ndarray:
    ...
    Inherited, k groupings
        See :func:`make_radial_trajectory`.

    Returns
    -------
    numpy.ndarray
        Set of trajectories as generated by :func:`make_radial_trajectory`.
    ...
    """
    lhs = pyDOE.lhs(k, 2 * r, 'cm')

    reps = []
    for i in range(r):
        a = lhs[i]
        b = lhs[i + 1]
        reps.append(make_radial_trajectory(k, groupings, a, b))
...
def make_winding_stair_trajectory_set(r: int, set_size: int, k: int, levels: int,
    ...
    -------
    numpy.ndarray
        Set of trajectories as generated by :func:`make_winding_stair_trajectory`.
